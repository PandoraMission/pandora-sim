...
d = loadmat('/Users/chedges/Downloads/pandora_nir_20220506_thin_prism_cold_PSF_512.mat')
nbin = 2
...
# Bin down wavelength too, also too big.
nbin = 20
PSF_cold = np.asarray([PSF_cold[:, :, idx::nbin] for idx in range(nbin)]).mean(axis=(0))
PSF_hot = np.asarray([PSF_hot[:, :, idx::nbin] for idx in range(nbin)]).mean(axis=(0))
wvl = np.asarray([d['wvl'][:900][idx::nbin] for idx in range(nbin)]).mean(axis=(0))[:, 0] * u.micron
temp = np.asarray([-10., 30.])*u.deg_C
wvl = wvl[:, None] * np.ones(len(temp))
