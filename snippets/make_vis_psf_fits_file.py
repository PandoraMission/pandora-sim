...
# hdu.writeto('/Users/chedges/repos/pandora-sim/src/pandorasim/data/pandora_vis_20220506.fits', overwrite=True)
...
# hdu.writeto('/Users/chedges/repos/pandora-sim/src/pandorasim/data/pandora_nir_20220506.fits', overwrite=True)

# Vis PSF with thermal info
# -----------------------------------#
...
PSF_hot = np.asarray(
    [
        [PSF_hot[idx::nbin, jdx::nbin] for idx in range(nbin)]
        for jdx in range(nbin)
    ]
).mean(axis=(0, 1))
...
PSF_cold = np.asarray(
    [
        [PSF_cold[idx::nbin, jdx::nbin] for idx in range(nbin)]
        for jdx in range(nbin)
    ]
).mean(axis=(0, 1))
...
hdu.writeto(
    "/Users/chedges/repos/pandora-sim/src/pandorasim/data/pandora_vis_20220506.fits",
    overwrite=True,
)
...
d = loadmat(
    "/Users/chedges/Downloads/pandora_nir_20220506_thin_prism_hot_PSF_512.mat"
)
nbin = 2
...
d = loadmat(
    "/Users/chedges/Downloads/pandora_nir_20220506_thin_prism_cold_PSF_512.mat"
)
nbin = 2
...
wvl = (
    np.asarray([d["wvl"][:900][idx::nbin] for idx in range(nbin)]).mean(
        axis=(0)
    )[:, 0]
    * u.micron
)
...
hdu.writeto(
    "/Users/chedges/repos/pandora-sim/src/pandorasim/data/pandora_nir_20220506.fits",
    overwrite=True,
)
