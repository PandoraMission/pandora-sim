...
class PSF(object):
    ...
    def __init__(self, filename=f"{PACKAGEDIR}/data/pandora_vis_20220506.fits"):
        ...
        [setattr(self, i.name.lower(), i.data * u.Unit(i.header['UNIT'])) for i in hdu[2:]]
        [setattr(self, f'{i.name.lower()}p', i.data * u.Unit(i.header['UNIT'])) for i in hdu[2:]]
        
        if self.ndims == 1:
            setattr(self, self.dimension_names[0] + '1d', getattr(self, self.dimension_names[0]))
            ...
            for dim in np.arange(self.ndims):
                lp = getattr(self, self.dimension_names[dim]).transpose(np.hstack([dim, list(dims - set([dim]))]))
        ...
        self.psf_x = ((np.arange(self.shape[0]) - self.shape[0]//2) * u.pixel * self.sub_pixel_size)/ self.pixel_size
        self.psf_y = ((np.arange(self.shape[1]) - self.shape[1]//2) * u.pixel * self.sub_pixel_size)/ self.pixel_size
        self.midpoint = tuple([getattr(self, name + '0d') for name in self.dimension_names])
    ...
    def psf(self, point, freeze_dimensions=None):
        ...
        PSF0 = self.psf_flux
        for dim, p in enumerate(point):
            PSF0 = interpfunc(p.value, getattr(self, self.dimension_names[dim] + '1d').value, PSF0)
        return PSF0
